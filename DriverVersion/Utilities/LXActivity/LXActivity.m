...
#define SHARETITLE_FONT                         [UIFont systemFontOfSize:14]
...
#define BUTTON_BORDER_COLOR                     [UIColor  lightTextColor].CGColor
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle shareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray
{
    ...
    if ( shareButtonImagesNameArray ) {
        if ( shareButtonImagesNameArray > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                ...
                UIImage * btnImage =[UIImage imageNamed:[shareButtonImagesNameArray objectAtIndex:i-1]];
                
                if (![TencentOAuth iphoneQQInstalled]&&(i==3||i==4)) {
//                    btnImage = [btnImage transToGrayImage];
...
//                        btnImage = [btnImage transToGrayImage];
    ...
    if (cancelButtonTitle) {
        ...
        UIView * bottomView = [[UIView alloc]initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, BUTTON_INTERVAL_HEIGHT, BUTTON_WIDTH, BUTTON_HEIGHT)];
        bottomView.backgroundColor = CANCEL_BUTTON_COLOR;
        UIView * lineView = [[UIView alloc]initWithFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, 0, BUTTON_WIDTH, 1)];
      
        lineView.backgroundColor =  [UIColor colorWithRed:243/255.00f green:244/255.00f blue:242/255.00f alpha:1];
...
//            [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x, self.LXActivityHeight, cancelButton.frame.size.width, cancelButton.frame.size.height)];
            [bottomView setFrame: CGRectMake(cancelButton.frame.origin.x, self.LXActivityHeight, cancelButton.frame.size.width, cancelButton.frame.size.height)];
...
- (UIButton *)creatCancelButtonWith:(NSString *)cancelButtonTitle
{
    ...
    [cancelButton setFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, 0, BUTTON_WIDTH, BUTTON_HEIGHT)];
...
//    cancelButton.layer.borderColor = BUTTON_BORDER_COLOR;
    ...
    [cancelButton setTitleColor: [UIColor colorWithRed:37/255.00f green:114/255.00f blue:254/255.00f alpha:1] forState:UIControlStateNormal];
