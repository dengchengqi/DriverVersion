    ...
    if ( self )
    {
        ...
        if (AlertViewType_LogoInstructions == alertType) {
            ...
            if (self.inputHandler )
            {
                ...
                self.inputView.font = fontSize_14;
                self.inputView.layer.masksToBounds = YES;
                self.inputView.layer.cornerRadius =  40/2;
                self.inputView.backgroundColor = UIColorFromRGB(0xEAEAEA);
...
- (void)initButtonView:( MASViewAttribute *)lastAttribute withConfig:(AlertViewConfig *)config withButtonItems:(NSArray *)items withBottomType:(AlertBottomViewType  )bottomType{
    ...
    for ( NSInteger i = 0 ; i < items.count; ++i )
    {
        ...
        [btn setBackgroundImage:[UIImage mm_imageWithColor:config.itemNormalColor] forState:UIControlStateNormal];
        [btn setBackgroundImage:[UIImage mm_imageWithColor:config.itemNormalColor] forState:UIControlStateHighlighted];
        
        [btn setTitle:item.title forState:UIControlStateNormal];
        [btn setTitleColor: config.itemNormalTitleColor forState:UIControlStateNormal];
        btn.layer.masksToBounds = YES;
        //            if (bottomType == AlertBottomViewType_buttonWidthCustom) {
        //                btn.layer.cornerRadius = config.buttonHeight/2;
        //            }else{
        //                btn.layer.cornerRadius = config.itemCornerRadius;
        //
        //            }
        btn.layer.cornerRadius = config.buttonHeight/2;
        btn.layer.borderWidth = MM_LAYER_SPLIT_WIDTH;
        btn.layer.borderColor = config.itemNormalLayerColor.CGColor;
    ...
    [lastButton setBackgroundImage:[UIImage mm_imageWithColor:config.itemPressedColor] forState:UIControlStateNormal];
    [lastButton setBackgroundImage:[UIImage mm_imageWithColor:config.itemPressedColor] forState:UIControlStateHighlighted];
    
    [lastButton setTitleColor:config.itemPressedTitleColor forState:UIControlStateNormal];
    
    
    lastButton.layer.borderWidth = MM_SPLIT_WIDTH;
    lastButton.layer.borderColor = config.itemPressedLayerColor.CGColor;
...
- (void)configSubjectsContentView:(NSString *)normal contentHeight:(CGFloat)contentHeight{
    ...
    for (int i = 0; i< 4; i++) {
        ...
        [btn setTitleColor:UIColorFromRGB(0x898989) forState:UIControlStateNormal];
        [btn setTitleColor:project_main_blue forState:UIControlStateSelected];
        btn.titleLabel.font = fontSize_14;
        btn.layer.masksToBounds = YES;
        btn.layer.borderColor =  UIColorFromRGB(0x898989).CGColor;
        btn.layer.borderWidth = 0.5;
        btn.layer.cornerRadius = 4;
        [btn  setBackgroundImage:[ProUtils createImageWithColor:[UIColor clearColor] withFrame:CGRectMake(0, 0, 1, 1)] forState:UIControlStateNormal];
        [btn  setBackgroundImage:[ProUtils createImageWithColor:UIColorFromRGB(0xD3E5FF) withFrame:CGRectMake(0, 0, 1, 1)] forState:UIControlStateSelected];
        [btn addTarget:self action:@selector(subjectAction:) forControlEvents:UIControlEventTouchUpInside];
        
        [self.contentView addSubview:btn];
        
        UIImageView * arrowImgV = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"subjects_new_selected"]];
        ...
        if (i == 0) {
            btntitle = @"语文";
            tag = ChineseBtnTag;
             arrowImgV.tag = ChineseChooseImgVTag;
            [self setupTopLeftBtn:btn withChooseImg: arrowImgV];
        }else if (i == 1){
            btntitle = @"数学";
            tag = MathematicsBtnTag;
             arrowImgV.tag = MathematicsChooseImgVTag;
            [self setupTopRightBtn:btn withChooseImg: arrowImgV];
        }else if (i == 2){
            btntitle = @"英语";
            tag = EnglishBtnTag;
             arrowImgV.tag = EnglishChooseImgVTag;
            [self setupBottomLeftBtn:btn withChooseImg: arrowImgV];
        }
        else if (i == 3){
            
            btntitle = @"其它";
            tag = OtherBtnTag;
            arrowImgV.tag = OtherChooseImgVTag;
            arrowImgV.hidden = NO;
            [self setupBottomRightBtn:btn withChooseImg: arrowImgV];
            btn.selected = YES;
        }
        btn.tag = tag;
...
- (void)setupTopLeftBtn:(UIButton *)btn  withChooseImg:(UIImageView *)arrowImgV {
     ...
     [arrowImgV mas_updateConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(self.contentView.mas_centerX).offset(0);
...
- (void)setupTopRightBtn:(UIButton *)btn  withChooseImg:(UIImageView *)arrowImgV {
    
    [btn mas_makeConstraints:^(MASConstraintMaker *make) {
        
        make.left.mas_equalTo(self.contentView.mas_centerX).offset(10);
...
- (void)setupBottomLeftBtn:(UIButton *)btn  withChooseImg:(UIImageView *)arrowImgV{
...
- (void)setupBottomRightBtn:(UIButton *)btn  withChooseImg:(UIImageView *)arrowImgV {
