...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([_titles[indexPath.row] isKindOfClass:[NSAttributedString class]]) {
        ...
        if ([_images[indexPath.row] isEqualToString:@"student_homework_screening_nor_results"]) {
            cell.textLabel.textColor = UIColorFromRGB(0xb1b1b1);
         
            //中划线
            NSDictionary *attribtDic = @{NSStrikethroughStyleAttributeName: [NSNumber numberWithInteger:NSUnderlineStyleSingle],NSStrokeColorAttributeName:UIColorFromRGB(0xb1b1b1), NSBaselineOffsetAttributeName:@(0)};
            
            NSMutableAttributedString *attribtStr = [[NSMutableAttributedString alloc]initWithString:_titles[indexPath.row] attributes:attribtDic];
