...
NSInteger  BottomToolBarHeight = 0;
...
//    self.navigationItem.title = _model.name;
    ...
    UIBarButtonItem *backItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"testreturn"] style:UIBarButtonItemStylePlain target:tzImagePickerVc action:@selector(cancelButtonClick)];
    self.navigationItem.leftBarButtonItem = backItem;
    _doneButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _doneButton.frame = CGRectMake(0, 3,FITSCALE(64), 44);
    _doneButton.titleLabel.font = rightBarButtonItem_font;
    [_doneButton addTarget:self action:@selector(doneButtonClick) forControlEvents:UIControlEventTouchUpInside];
//    [_doneButton setTitle:tzImagePickerVc.doneBtnTitleStr forState:UIControlStateNormal];
//    [_doneButton setTitle:tzImagePickerVc.doneBtnTitleStr forState:UIControlStateDisabled];
    
    
     [_doneButton setTitle:[NSString stringWithFormat:@"%@(%zd/%zd)",tzImagePickerVc.doneBtnTitleStr,tzImagePickerVc.selectedModels.count,tzImagePickerVc.maxImagesCount] forState:UIControlStateNormal];
    ...
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc]initWithCustomView:_doneButton];
     rightItem.enabled = tzImagePickerVc.selectedModels.count || tzImagePickerVc.alwaysEnableDoneBtn;
...
- (void)configCollectionView {
    ...
    if (self.navigationController.navigationBar.isTranslucent) {
        top = 44;
        if (iOS7Later) top += 20;
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.tz_height - BottomToolBarHeight - top : self.view.tz_height - top;
    } else {
        CGFloat navigationHeight = 44;
        if (iOS7Later) navigationHeight += 20;
        collectionViewHeight = tzImagePickerVc.showSelectBtn ? self.view.tz_height - BottomToolBarHeight - navigationHeight : self.view.tz_height - navigationHeight;
    }
...
- (void)configBottomToolBar {
    ...
    CGFloat yOffset = 0;
    if (self.navigationController.navigationBar.isTranslucent) {
        yOffset = self.view.tz_height - BottomToolBarHeight;
    } else {
        CGFloat navigationHeight = 44;
        if (iOS7Later) navigationHeight += 20;
        yOffset = self.view.tz_height - BottomToolBarHeight - navigationHeight;
    }
    
    UIView *bottomToolBar = [[UIView alloc] initWithFrame:CGRectMake(0, yOffset, self.view.tz_width, BottomToolBarHeight)];
    ...
    _doneButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _doneButton.frame = CGRectMake(self.view.tz_width - 44 - 10, 3, 44, 44);
...
//        [alert show];
        ...
        NSArray * items =  @[MMItemMake(@"取消", MMItemTypeHighlight, nil),
                             MMItemMake(@"设置", MMItemTypeHighlight, itemHandler)];
        AlertView * alertView = [[AlertView alloc]initWithTitle:[NSBundle tz_localizedStringForKey:@"Can not use camera"] detail:message items:items];
        [alertView show];
...
- (void)refreshBottomToolBarStatus {
    ...
    self.navigationItem.rightBarButtonItem.enabled =  tzImagePickerVc.selectedModels.count > 0 || tzImagePickerVc.alwaysEnableDoneBtn;
    NSString * selectedCount = [NSString stringWithFormat:@"%zd",tzImagePickerVc.selectedModels.count];
    
    [_doneButton setTitle:[NSString stringWithFormat:@"%@(%@/%zd)",tzImagePickerVc.doneBtnTitleStr,selectedCount,tzImagePickerVc.maxImagesCount] forState:UIControlStateNormal];
...
//            [alert show];
            AlertView * alert = [[AlertView alloc]initWithOperationState:TNOperationState_Unknow detail:message items:nil];
            [alert show];
        }
    }
}
